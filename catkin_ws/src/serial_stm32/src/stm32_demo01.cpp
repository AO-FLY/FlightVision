...
void send_odom_yaw(float t265x,float t265y,float yaw);
void send_odom_yaw(float t265x,float t265y,float yaw)
{
    ...
    for(i=0;i<4;i++)
    {
        buffer[i+2]=t265_x.data[i]; 
        buffer[i+6]=t265_y.data[i];
       buffer[i+10]=t265_yaw.data[i];
   }
       ros_send.write(buffer,14);
}

void doMsg(const nav_msgs::Odometry::ConstPtr& msg)
{
  static float pose_x=0,pose_y=0,pose_z=0,speed_x=0,speed_y=0,speed_z=0,angle_yaw=0;
...
angle_yaw=get_yaw(msg);
//  write_odom(pose_x,pose_y,pose_z, speed_x, speed_y, angle_yaw);
send_odom_yaw(pose_x,pose_y,angle_yaw);
 //ROS_INFO("yaw坐标:%f",  angle_yaw);
...
int main(int argc, char  *argv[])
{
    ...
    ros_send.setPort("/dev/ttyTHS1");
    ros_send.setBaudrate(576000);
    ...
    if(ros_send.isOpen())
    {
        ...
        ROS_INFO_STREAM("Serial Port  has opened");
    }
    else
    {
        return -1;
